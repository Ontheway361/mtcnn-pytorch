...
class DetectFace(object):
    ...
    def  __init__(self, min_face = 12, use_cuda = False):

        self._create_mtcnn_net(use_cuda)
    ...
    def _create_mtcnn_net(self, use_cuda = False):
        ''' Equip the basenet for detector '''

        # PNet
        self.pnet_detector = PNet(use_cuda=use_cuda)
        self.pnet_detector.load_state_dict(torch.load('checkpoint/pnet.pt', \
                                                       map_location=lambda storage, loc: storage))
        if use_cuda:
            self.pnet_detector = self.pnet_detector.cuda()
        self.pnet_detector.eval()

        # RNet
        self.rnet_detector = RNet(use_cuda=use_cuda)
        self.rnet_detector.load_state_dict(torch.load('checkpoint/rnet.pt', \
                                                       map_location=lambda storage, loc: storage))
        if use_cuda:
            self.rnet_detector = self.rnet_detector.cuda()
        self.rnet_detector.eval()

        # ONet
        self.onet_detector = ONet(use_cuda=use_cuda)
        self.onet_detector.load_state_dict(torch.load('checkpoint/onet.pt', \
                                                       map_location=lambda storage, loc: storage))
        if use_cuda:
            self.onet_detector = self.onet_detector.cuda()
